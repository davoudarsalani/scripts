...
from gp import Color, pip_to_fzf, invalid, get_input, hash_string, hash_file
...
def prompt(*args: list[str]) -> None:  ## {{{
    ...
    for arg in args:
        if   arg == '-f':
            ...
            if not path.exists(file): invalid(f'{file} does not exist')
            if path.isdir(file): invalid('files only')
        elif arg == '-s':
            try:    string
