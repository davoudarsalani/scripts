...
##    https://github.com/davoudarsalani/scripts/blob/master/git.sh
            ...
            y )
                {
                    ...
                    printf '%s git -C %s push --set-upstream origin master\n' "$(blue 'push to remote')" "$(to_tilda "$test_dir")"  ## --set-upstream or -u
...
if [ "$directory" ]; then
    ...
    directory="$(select_directory 'git')" && wrap_fzf_choice "$(to_tilda "$directory")" || exit 37
